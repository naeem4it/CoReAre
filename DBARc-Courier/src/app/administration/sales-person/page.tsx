...
export default function AdministrationSalesPersonPage() {
  const { user } = useAuth();
  const router = useRouter();

  // Courier Admin Access Check
  const isCourierAdmin = React.useMemo(() => {
    if (!user) return false;
    const isShipper = (
      (user.shipper_roles && user.shipper_roles.length > 0) ||
      (user.user_type === 'shipper' || user.type === 'shipper') ||
      (user.role?.name || user.role_type || '').toString().toLowerCase().includes('shipper')
    );
  ...
  const [salesPersons, setSalesPersons] = React.useState<SalesPersonRecord[]>([]);
  ...
  const [editingSalesPersonId, setEditingSalesPersonId] = React.useState<number | null>(null);
  ...
  // Fetch all salesPersons and courier employees
  const fetchData = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const [salesPersonsRes, usersRes] = await Promise.allSettled([
        apiClient.get('/sales-persons?populate=*&pagination[pageSize]=500&sort=createdAt:desc'),
        apiClient.get('/users?populate=role_definition,tenant,role&pagination[pageSize]=500')
      ]);

      if (salesPersonsRes.status === 'fulfilled') {
        const rawList = salesPersonsRes.value.data?.data || [];
        const mappedSalesPersons: SalesPersonRecord[] = rawList.map((item: any) => ({
          id: item.id,
          name: item.name || item.attributes?.name || `Sales Person #${item.id}`,
      ...
      console.warn('Failed to load salesPersons or employees:', err);
    } finally {
      setIsLoading(false);
    }
  }, [currentTenantId]);
  ...
  // Open Edit Form
  const handleOpenEdit = (salesPerson: SalesPersonRecord) => {
    setEditingSalesPersonId(salesPerson.id);
    setFormError(null);
    const type = salesPerson.entity_type === 'Our Employee' ? 'Our Employee' : 'Contractor';
    setEntityType(type);
    setSelectedEmployeeId(salesPerson.employee?.id ? String(salesPerson.employee.id) : '');
    setFormName(salesPerson.name || '');
    setFormEmail(salesPerson.email || '');
    setFormPhone(salesPerson.phone || '');
    setFormAddress(salesPerson.address || '');
    setProfitType(salesPerson.profit_type === 'Percentage' ? 'Percentage' : 'Fixed price');
    setProfitValue(String(salesPerson.profit_value ?? 50));
    setFormStatus(salesPerson.status || 'active');
    setIsFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (entityType === 'Our Employee') {
      ...
      if (!formName.trim()) {
        setFormError('Sales Person Name is mandatory for Contractors.');
        return;
      }
      if (!formAddress.trim()) {
    ...
    try {
      ...
      if (formPhone.trim()) dataObj.phone = formPhone.trim();
      if (formEmail.trim()) dataObj.email = formEmail.trim();
      if (formAddress.trim()) dataObj.address = formAddress.trim();

      if (currentTenantId) {
        dataObj.tenant = Number(currentTenantId);
      }

      if (entityType === 'Our Employee' && selectedEmployeeId) {
        dataObj.employee = Number(selectedEmployeeId);
      }

      const payload = { data: dataObj };

      if (editingSalesPersonId) {
        await apiClient.put(`/sales-persons/${editingSalesPersonId}`, payload);
        triggerToast(`Sales Person "${formName}" updated successfully!`, 'success');
      } else {
        await apiClient.post('/sales-persons', payload);
        triggerToast(`Sales Person "${formName}" registered successfully!`, 'success');
      }
      ...
      const detailedMsg = err.response?.data?.error?.details?.key
        ? `Validation error on ${err.response.data.error.details.key}: ${err.response.data.error.message}`
        : (err.response?.data?.error?.message || err.message || 'Failed to save salesPerson in database.');
  ...
  const handleDelete = async (salesPerson: SalesPersonRecord) => {
    if (!confirm(`Are you sure you want to remove salesPerson "${salesPerson.name}"?`)) return;
    try {
      await apiClient.delete(`/sales-persons/${salesPerson.id}`);
      triggerToast(`Sales Person "${salesPerson.name}" removed.`, 'success');
      await fetchData();
    } catch (err: any) {
      alert(err.response?.data?.error?.message || 'Failed to delete salesPerson.');
    }
  };
  ...
  const filteredSalesPersons = React.useMemo(() => {
    ...
    if (!isLoading && !isCourierAdmin) {
    return (
      <PortalLayout>
        <div className="max-w-4xl mx-auto p-8 my-12 bg-white border border-rose-200 rounded-3xl shadow-sm text-center space-y-4">
          ...
          <p className="text-sm text-slate-600 max-w-md mx-auto">
            The <strong>Sales Person</strong> screen is strictly accessible to <strong>Courier Administrators</strong> only.
          </p>
          <div className="pt-4">
            <button
              onClick={() => router.push('/')}
              className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all inline-flex items-center gap-2 cursor-pointer shadow-sm"
            >
  ...
  return (
    <PortalLayout>
      ...
      <div className="space-y-6 max-w-[1920px] w-full mx-auto p-lg pb-16">
        ...
        <div className="bg-slate-900 text-white p-5 rounded-3xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <Building2 className="w-4 h-4 text-primary-400" /> Administration / Sales Persons Setup
            </div>
            <h1 className="text-xl font-bold tracking-tight mt-0.5">Sales Person Accounts &amp; Profit Engine</h1>
            <p className="text-xs text-slate-400 mt-1">
              Configure salesPersons as Courier Employees or Independent Contractors with automated profit calculation rates.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
              <div>
                <h2 className="text-lg font-bold text-slate-900">
                  {editingSalesPersonId ? 'Edit Sales Person Configuration' : 'Register New Sales Person'}
                </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-2.5">
                  Select Sales Person Entity Type
                </label>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-slate-500">Internal courier employee operating as salesPerson</div>
                      </div>
                    </div>
                  </label>
              ...
              {/* Step 2: Sales Person Information Textboxes */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Sales Person Name */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                    Sales Person / Business Name {entityType === 'Contractor' && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    type="email"
                    placeholder="e.g. salesPerson@example.com"
                    value={formEmail}
                    disabled={entityType === 'Our Employee'}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                >
                  <Save className="w-4 h-4" /> {isSubmitting ? 'Saving...' : editingSalesPersonId ? 'Update Sales Person' : 'Save Sales Person'}
                </button>
              </div>
            </form>
          </div>
        ...
        {/* Registered Sales Persons Table */}
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-sm font-bold text-slate-900">Registered Sales Persons Directory</h2>
              <p className="text-xs text-slate-500">
                Total {filteredSalesPersons.length} merchant accounts registered for this courier tenant.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-bold border-b border-slate-200 text-[10px]">
                <tr>
                  ...
                  <th className="py-3 px-4">Sales Person / Brand</th>
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={9} className="py-12 text-center text-slate-400">
                      ...
                      Loading salesPersons directory...
                    </td>
                  </tr>
                ) : filteredSalesPersons.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="py-12 text-center text-slate-400">
                      No salesPersons found. Click <strong>+ Add Sales Person</strong> above to register a new account.
                    </td>
                  </tr>
                ) : (
