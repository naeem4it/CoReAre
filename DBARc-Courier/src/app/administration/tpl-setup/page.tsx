...
export default function TPLSetupPage() {
  const { user } = useAuth();
  ...
  const fetchPartners = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load tpl partners from database:', err);
      setPartners([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSavePartner = async () => {
    if (!tenantId) {
      setNotification({ type: 'error', text: 'No active courier organization found.' });
      return;
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 to-indigo-950 p-6 rounded-2xl text-white shadow-xl">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-200 text-xs font-semibold backdrop-blur-md">
              ...
              3PL Logistics Integrations
            </div>
            <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
              3PL Partner Setup
            </h1>
            <p className="text-sm text-indigo-100/80 max-w-2xl">
              Configure and authenticate third-party logistics courier APIs (Trax, PostEx, Leopards, TCS, M&P, CallCourier, etc.). Deliveries destined outside your in-house 2PL service areas will be automatically booked through these partners.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm shadow-lg shadow-blue-500/30 flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              ...
              Active 3PL Partners ({partners.length})
            </h2>
            <span className="text-xs text-slate-500 font-medium">
              Preferred partner acts as primary fallback for non-2PL orders
            </span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 space-y-3">
              <div className="w-14 h-14 mx-auto rounded-2xl bg-blue-50 dark:bg-blue-950/40 flex items-center justify-center text-blue-600 dark:text-blue-400">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Connect your accounts for Trax, PostEx, Leopards, TCS, M&P or other couriers to enable automated order booking outside your 2PL territory.
              </p>
              <button
                onClick={() => openEditModal()}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {partners.map(partner => {
                ...
                return (
                  <div
                    key={partner.id}
                    className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4"
                  >
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-400">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-semibold uppercase tracking-wider text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
              ...
              <div className="p-6 space-y-5 overflow-y-auto flex-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  >
                    {TPL_PROVIDERS.map(p => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    />
                  </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-3">
                      ...
                      <div key={field.key} className="space-y-1">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full pl-3.5 pr-10 py-2 text-xs font-mono bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500"
                          />
                          {isSecret && (
                            <button
                              type="button"
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <button
                      ...
                      className="px-4 py-2 bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                    >
                ...
                {/* Preferred 3PL Switch */}
                <label className="flex items-center gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer">
                  <input
                    type="checkbox"
                    checked={isPreferred}
                ...
                {/* City Code Mapping Drawer / Section */}
                <div className="p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-xl space-y-3">
                  ...
                  <div className="max-h-44 overflow-y-auto space-y-2 pr-1 text-xs">
                    {['Lahore', 'Karachi', 'Islamabad', 'Rawalpindi', 'Faisalabad', 'Multan', 'Peshawar', 'Quetta', 'Sialkot', 'Gujranwala'].map(city => {
                      ...
                      return (
                        <div key={city} className="flex items-center justify-between p-2 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              className="w-20 px-2 py-1 text-xs font-mono font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-center focus:ring-1 focus:ring-blue-500 focus:outline-none"
                            />
                          </div>
                        </div>
                      );
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold shadow-lg shadow-blue-500/20 flex items-center gap-2 transition-all disabled:opacity-50"
                >
                  {isSaving ? (
                    <>
