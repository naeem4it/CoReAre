...
import { Search, ChevronLeft, ChevronRight, Package, Calendar, BadgeDollarSign, User, Edit } from 'lucide-react';
...
const columnHelper = createColumnHelper<CourierShipment>();

export const CourierShipmentsTable = () => {
  const [data, setData] = React.useState<CourierShipment[]>([]);
    ...
    const fetchShipments = async () => {
      try {
        ...
        const formattedData = response.data.data.map((item: any) => ({
          ...
          rider: item.rider?.name || 'Unassigned',
        }));
      ...
      cell: (info) => (
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => {
              // TODO: Assign rider
              console.log('Assign rider to:', info.row.original.trackingId);
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              // TODO: Update status
              console.log('Update status for:', info.row.original.trackingId);
            }}
          >
