...
export default function AdminTPLSetupPage() {
  ...
  const [isLoadingPartners, setIsLoadingPartners] = React.useState<boolean>(false);
  ...
  const fetchTenants = async () => {
    try {
      ...
      console.error('Failed to fetch tenants from database:', err);
      setTenants([]);
    } finally {
  ...
  // Fetch 3PL partners for selected tenant from database
  const loadPartners = React.useCallback(async (tenantId: string) => {
    if (!tenantId) return;
    try {
  ...
  React.useEffect(() => {
    if (selectedTenantId) {
      loadPartners(selectedTenantId);
    }
  }, [selectedTenantId, loadPartners]);
  ...
  const handleSavePartner = async () => {
    if (!selectedTenantId) {
      setNotification({ type: 'error', text: 'Please select a tenant organization.' });
      return;
    }
    ...
    try {
      ...
      await loadPartners(selectedTenantId);
  ...
  const handleTogglePreferred = async (partnerId: string | number) => {
    try {
      ...
      console.error('Failed to set preferred partner:', err);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 to-indigo-950 p-6 rounded-2xl text-white shadow-xl">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-200 text-xs font-semibold backdrop-blur-md">
            ...
            Super Administration &bull; 3PL Ecosystem
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
            3PL Partner Integrations Setup
          </h1>
          <p className="text-sm text-indigo-100/80 max-w-2xl">
            Configure, authenticate, and manage third-party logistics APIs (Trax, PostEx, Leopards, TCS, M&P, CallCourier, etc.) on behalf of any registered courier tenant.
          </p>
        </div>
        ...
        <CardContent className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
              ...
              Select Courier Tenant *
            </label>
            <p className="text-xs text-slate-500">
              Select the courier business whose 3PL partner integrations you want to view or manage.
            </p>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 font-medium">
            Tenant: {tenants.find(t => t.id === selectedTenantId)?.name || 'Selected Courier'}
          </span>
        </div>
        ...
        {isLoadingPartners ? (
          <div className="p-12 text-center text-slate-400 bg-white rounded-2xl border border-slate-200">
            ...
            Loading 3PL integrations for selected tenant...
          </div>
        ) : partners.length === 0 ? (
          <div className="p-12 text-center bg-white rounded-2xl border border-dashed border-slate-300 space-y-3">
            ...
            <h3 className="text-base font-semibold text-slate-800">
              No 3PL Partners Configured for this Tenant
            </h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Configure credentials for Trax, PostEx, Leopards, TCS, M&P, or CallCourier to enable automated fallback routing.
            </p>
            <Button
              onClick={() => openEditModal()}
              className="bg-blue-600 text-white rounded-xl text-xs font-semibold hover:bg-blue-700"
            >
              Connect First 3PL Partner
            </Button>
          </div>
        ) : (
            ...
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Configuring for: {tenants.find(t => t.id === selectedTenantId)?.name}
                  </p>
                </div>
              </div>
              <button
