...
export const PakistanLocationSelect: React.FC<PakistanLocationSelectProps> = ({
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`} ref={containerRef}>
      ...
      <div className="relative">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className={`w-full text-xs pl-9 pr-8 py-2 bg-white border rounded-xl outline-none transition-all ${
              error 
                ? 'border-red-400 focus:ring-2 focus:ring-red-400/20' 
                : 'border-slate-200 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500'
        ...
        {openDropdown === 'search' && directSearchResults.length > 0 && (
          <div className="absolute left-0 right-0 top-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-64 overflow-y-auto divide-y divide-slate-100">
            <div className="px-3 py-2 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500">
            ...
            {directSearchResults.map((loc, idx) => (
              <div
                ...
                className="px-3 py-2 text-xs hover:bg-primary-50 cursor-pointer flex items-center justify-between group transition-colors"
              >
                <div className="flex items-center gap-2">
                  <MapPin className="w-3.5 h-3.5 text-primary-600 shrink-0" />
                  <div>
                    <span className="font-bold text-slate-900 group-hover:text-primary-600">
                      {loc.tehsil}
                    </span>
      ...
      <div className={`grid ${layout === 'horizontal' ? 'grid-cols-1 sm:grid-cols-3' : 'grid-cols-1'} gap-2.5 mt-0.5`}>
        ...
        <div className="relative">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2 text-xs bg-white border rounded-xl cursor-pointer transition-all ${
              selectedProvince
                ? 'border-slate-300 text-slate-900 font-semibold'
        ...
        <div className="relative">
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2 text-xs bg-white border rounded-xl cursor-pointer transition-all ${
              selectedTehsil
                ? 'border-primary-500/50 text-slate-900 font-bold bg-primary-50/20'
                : 'border-slate-200 text-slate-400'
            } hover:border-slate-400`}
          >
      ...
      {isComplete && (
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl text-[11px] text-emerald-800 font-semibold animate-in fade-in duration-200">
